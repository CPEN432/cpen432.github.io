...
/* Chronos notice:
 *------------------------------------------------------------------------------
 * We have made some modifications to this file for the following purposes:
 *
 * - to only count branch mispredictions in the user code in the address range
 *   [ start_ddr, end_addr] (library mispredictions are thus not counted)
 *   in a variable effect_bpred_misses
 *
 * - SimpleScalar uses two-bit saturating counter for various branch prediction
 *   schemes, whereas we only model one-bit counter in this release, therefore
 *   we changed the tow-bit counter to one-bit in corresponding locations, the
 *   places where the changes are made have been given annotations starting with
 *   "Chronos".
 *   
 *------------------------------------------------------------------------------
 */
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  case BPred2Level:
	...
	fatal("level-2 size, `%d', must be non-zero and a power of two", 
      ...
      pred_dir->config.two.l2width = 0;
      int tmp_l2size;
      for (tmp_l2size = l2size; tmp_l2size > 1; tmp_l2size >>= 1)
	  pred_dir->config.two.l2width++;
    ...
    */
      for (cnt = 0; cnt < l2size; cnt++)
	  pred_dir->config.two.l2table[cnt] = 0;
  ...
  case BPred2bit:
    ...
    */
    for (cnt = 0; cnt < l2size; cnt++)
      pred_dir->config.bimod.table[cnt] = 0;
...
#else
	    //l2index = l2index ^ (baddr >> MD_BR_SHIFT);
	    l2index = (l2index << (pred_dir->config.two.l2width -
			pred_dir->config.two.shift_width)) ^ (baddr >> MD_BR_SHIFT);
#endif
	  }
	else
	  {
...
/* 
 * Chronos: the lines in the #if 0 are replaced by the one line following it to
 * capture the change from two-bit saturaing counter to one-bit counter
 */
#if 0
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if (!!pred_taken == !!taken)
    ...
    pred->misses++;
    if ((baddr >= start_addr) && (baddr < end_addr)) {
	effect_bpred_misses++;
	//printf("bpred_addr:%x; target:%x; pred:%d\n", baddr, btarget, pred_taken);
    }
  }
  ...
  // if a branch is not in [start_addr, end_addr), don't update the predictor
  if ((baddr < start_addr) || (baddr >= end_addr))
      return;
...
int  ii;
printf("%x:  ", baddr);
for (ii = 0; ii < pred->dirpred.twolev->config.two.l2size; ii++) {
    printf("%d ", pred->dirpred.twolev->config.two.l2table[ii]);
}
printf("\n");
#endif
