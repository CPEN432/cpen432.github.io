...
/* Chronos notice:
 *------------------------------------------------------------------------------
 * We have added code to infer the start/end addresses of the user code from the
 * executable, so that we will exclude analysis of code not in the range
 * [start_addr, end_addr]. The annotations in these places contain "Chronos"
 *------------------------------------------------------------------------------
 */
...
// Chronos: tell from the name of the function whether it is a library function,
// if so, it means no subsequent user functions
static int
...
// Chronos: get start/end addresses of the program by excluding library code
void
get_prog_range()
{
    int			i, size = 0;
	     ...
	     int load_locals)	/* load local symbols */
{
  ...
  // Chronos: to exclude library code from analysis
  get_prog_range();
}
