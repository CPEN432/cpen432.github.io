...
/*
   commented by Chronos: to disassemble the format used by Chronos
*/
	      ...
	      FILE *stream)		/* output stream */
{
  ...
  if (op == OP_NA || op >= OP_MAX)
	...
	case 'i':
	  ...
	  break;
	case 'H':
	  fprintf(stream, "0x%x", SHAMT);
	  break;
	case 'u':
	  fprintf(stream, "%u", UIMM);
	  break;
	case 'U':
	  fprintf(stream, "%d", UIMM);
	  break;
	case 'J':
	  fprintf(stream, "%08x", ((pc & 036000000000) | (TARG << 2)));
	  break;
	case 'B':
	  fprintf(stream, "%08x", BCODE);
	  break;
#if 0 /* FIXME: obsolete... */
	case ')':
